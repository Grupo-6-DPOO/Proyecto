...
public class PiezaEnInventario {
	...
	public PiezaEnInventario(Pieza pieza, int ubicacionEnGaleria, boolean enConsignacion,
			String salidaConsignacion, Cliente duenio, boolean puedeSalir, List<Cliente> historialDuenios, Integer id, List<Pago> pagosPieza) {
		super();
		this.pieza = pieza;
		...
		this.id = id;
		this.pagosPieza = pagosPieza;
		currentid = Integer.max(currentid,id)+1;
	}
	
	public PiezaEnInventario(Pieza pieza, int ubicacionEnGaleria, boolean enConsignacion,
			String salidaConsignacion, Cliente duenio, boolean puedeSalir, List<Cliente> historialDuenios, List<Pago> pagosPieza) {
		super();
		this.pieza = pieza;
		this.ubicacionEnGaleria = ubicacionEnGaleria;
		this.enConsignacion = enConsignacion;
	...
	public String toString() {
		...
		String historialPagosString = "{";
		for (Pago pago: this.getPagosPieza()) {
			historialPagosString += ("(Fecha: "+ pago.getFecha() + ", Monto: "+ pago.getMonto()+", Fecha del pago: "+ pago.getFecha()+"), ");
		}
			
		historialPagosString += "}";
		...
		return "[pieza=" + pieza.toString() + ", ubicacionEnGaleria=" + ubicacionEnGaleriastr + ", enConsignacion="
				+ enConsignacion + ", salidaConsignacion=" + salidaConsignacion + ", duenio=" + duenio.getUsername() + ", puedeSalir="
				+ puedeSalir + ", historialDuenios=" + historialDueniosString + ", id=" + id + ", Pagos=" + historialPagosString +"]";
	}
	
	
	public List<Pago> getPagosPieza() {
		return pagosPieza;
	}

	public void setPagosPieza(List<Pago> pagosPieza) {
		this.pagosPieza = pagosPieza;
	}
	
	public void agregarPago(Pago pago) {
		this.getPagosPieza().add(pago);
	}
