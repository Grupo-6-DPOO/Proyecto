...
public class InterfazCajero {
	...
	public static void menuRevisarPagos(Galeria galeria) {
		...
		if (r1==0 | pagosPendientes.size()==0) {
			...
			String fechaActual = pedirCadenaAlUsuario("Ingrese la fecha de hoy");
			...
			if (r2==1) {
				try {
					galeria.aprobarPago(pagoSeleccionado, Pago.TARJETA,fechaActual);
					menuRevisarPagos(galeria);
				} catch (Exception e) {
				...
				try {
					galeria.aprobarPago(pagoSeleccionado, Pago.TRANSFERENCIA,fechaActual);
					menuRevisarPagos(galeria);
				} catch (Exception e) {
					// TODO Auto-generated catch block
					System.out.println(e.getMessage());
				...
				try {
					galeria.aprobarPago(pagoSeleccionado, Pago.EFECTIVO,fechaActual);
					menuRevisarPagos(galeria);
				} catch (Exception e) {
					// TODO Auto-generated catch block
					System.out.println(e.getMessage());
